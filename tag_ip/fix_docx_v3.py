...
# ── Helpers ──
def make_para(text, bold=False, italic=False, size=11, align='left', font='Times New Roman'):
    p = OxmlElement('w:p')
...
UNNAMED_TABLE_INTERPS = {
    ...
    3: "Les équipements garantissent la protection du réseau et la continuité du service de suivi de 10 000 véhicules.",
    4: "L'environnement utilise PostgreSQL 16 avec PostGIS pour les opérations géospatiales.",
    5: "L'équipe du pôle Ingénierie Logicielle est structurée avec quatre profils complémentaires.",
    6: "Les problèmes d'incompatibilité matérielle et de dépendance humaine génèrent des surcoûts significatifs.",
    7: "Les coûts mensuels cachés s'élèvent à plusieurs millions d'Ariary.",
    8: "Le coût mensuel total estimé à 8 500 000 Ar justifie l'investissement dans une solution automatisée.",
    9: "Le projet a été organisé en quatre sprints d'une semaine selon la méthode agile.",
    10: "Six informateurs clés ont été interrogés pour recueillir les besoins et contraintes.",
    11: "TAG-Monitor se distingue par sa flexibilité et son approche sur mesure.",
    12: "L'automatisation réduit le temps de compatibilité de 30-45 min à 45 ms.",
    17: "L'architecture à trois couches assure une séparation claire des responsabilités.",
    18: "Les cas d'utilisation couvrent les interactions entre les acteurs et le système.",
    21: "La base de données utilise des UUID avec des index composites pour optimiser les requêtes.",
    22: "Les tables de jonction gèrent les relations many-to-many.",
    23: "La table mounting_profiles stocke les profils de montage avec leurs caractéristiques.",
    24: "La table modèles_traceur référence les modèles de traceurs GPS avec leurs spécifications.",
    25: "La table compatibilities enregistre les résultats des vérifications de compatibilité.",
    26: "Les couches applicatives détaillent l'organisation du code selon le pattern Phoenix/Ash.",
    29: "L'environnement utilise une stack Elixir 1.18, Phoenix 1.8, Ash 3.4 avec PostgreSQL 16.",
    35: "L'architecture déclarative Ash Framework réduit le code boilerplate et accélère le développement.",
}

NUMEROTED_TABLEAU_INTERPS = {
    13: "Trois profils (administrateur, technicien exploitation, technicien terrain) ont des droits d'accès distincts.",
    14: "Les contraintes couvrent performance, sécurité, compatibilité et évolutivité du système.",
    15: "Les six modules couvrent l'ensemble du périmètre fonctionnel, de la gestion des profils à l'export.",
    16: "Le choix d'Ash Framework est déterminant pour la productivité et la maintenabilité du projet.",
    19: "Le planning s'étend sur 8 semaines avec des jalons clés du cahier des charges aux tests.",
    20: "Les risques principaux incluent la courbe d'apprentissage Ash et l'instabilité du périmètre.",
    27: "Le comparatif montre qu'Ash Framework offre la meilleure flexibilité et un temps de développement réduit.",
    28: "Le scoring pondéré sur 100 points répartit 20 vérificateurs en 5 catégories.",
    30: "L'environnement de développement utilise Phoenix 1.8, Ash 3.4 avec PostgreSQL 16.",
    31: "La stratégie pyramide couvre les tests unitaires, d'intégration et fonctionnels.",
    32: "Les tests fonctionnels couvrent les parcours utilisateur critiques avec 10 scénarios validés.",
    33: "Le calcul de compatibilité s'effectue en 45 ms (couple) et 420 ms (batch), dépassant les objectifs.",
    34: "Les tests de charge confirment la stabilité du système avec 50 utilisateurs concurrents.",
    36: "Les variables d'environnement centralisent la configuration pour les déploiements multi-environnements.",
}
...
for i, (typ, elem, pidx, tidx) in enumerate(elem_info):
    if typ != 'tbl':
        continue

    # Scan backwards for preceding Tableau caption + Source
    last_cap = None
    last_cap_idx = None
    has_src = False
    ...
    for j in range(i - 1, -1, -1):
        ...
        if t == 'p':
            ...
            if not txt.startswith('Figure'):
                continue  # skip figure captions
        if t == 'tbl':
            break

    is_num = last_cap_idx in NUMEROTED_TABLEAU_PARAS

    # Scan ahead for interpretation
    has_interp = False
    for j in range(i + 1, min(i + 6, len(elem_info))):
        t, e, _, _ = elem_info[j]
        if t == 'p':
            txt = para_text(e)
            ...
            if txt == '':
                continue
            if txt.startswith(('Figure', 'Tableau', 'Source :')):
                continue
            break
        if t == 'tbl':
            break
    ...
    if is_num:
        ...
        if not has_interp and tidx in NUMEROTED_TABLEAU_INTERPS:
            elem.addnext(make_para(
                f"Interprétation : {NUMEROTED_TABLEAU_INTERPS[tidx]}",
                italic=True, size=11))
            added_interp += 1
        ...
        if tidx in UNNAMED_TABLE_TITLES:
            ...
            print(f"  T{tidx} nommé +label+src+interp")
        else:
            elem.addprevious(make_para("Tableau (non numéroté)", bold=True, size=11, align='center'))
            elem.addprevious(make_para("Source : Auteur, 2026", italic=True, size=10))
            elem.addnext(make_para("Interprétation : Ce tableau présente les données du système.", italic=True, size=11))
            added_label += 1; added_src += 1; added_interp += 1
            print(f"  T{tidx} générique +label+src+interp")

print(f"Tableaux : +{added_label} labels, +{added_src} sources, +{added_interp} interprétations")
...
for d in drawings:
    ...
    if not has_cap:
        p_elem.addprevious(make_para(f"Figure (non numérotée) n°{fig_count}", bold=True, size=11, align='center'))
        fig_label += 1
    if not has_src:
    ...
    print(f"  Figure {fig_count}: +{not has_cap} cap +{not has_src} src +{not has_interp} interp")

print(f"Figures : +{fig_label} labels, +{fig_src} sources, +{fig_interp} interprétations")

# ═══ PHASE 4: Fix paragraph alignment ═══
...
print(f"\nAlignement : {j} justifiés, {l} gauche (code), {s} conservés (titres/légendes)")
